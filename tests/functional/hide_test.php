...
class hide_test extends phpbb_functional_test_case
{
	...
	public function test_hide_inline_bbcode()
	{
		$post = $this->create_topic(
			2,
			'Hide Functional Test 2',
			'[hide inline=1]Hidden text[/hide]'
		);
		...
		$expected = '<span class="hidden-content" title="'.$this->lang('HIDDEN_CONTENT').'">'.
						'Hidden text'.
					'</span>';
	...
	/**
	 * @depends test_hide_inline_bbcode
	 */
	public function test_guest_hide_inline_bbcode()
	{
		$this->logout();
		...
		$expected = '<span class="hidden-content error">'.
						$this->lang('HIDDEN_CONTENT_EXPLAIN').
					'</span>';
